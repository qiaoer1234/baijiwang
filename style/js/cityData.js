...
y.indexbase = function () {
    ...
    var clicks = function () {
        $("#city_zm li").click(function () {
            $dom.css("marginLeft",'0')
            var value = $(this).html();
            $(".shj_yindao_citys_item_zm_city").html("");
            for (var i = 0; i < y.city.length; i++) {

                if (y.city[i].name == value) {
                    for (var j = 0; j < y.city[i].cities.length; j++) {
                        $(".shj_yindao_citys_item_zm_city").append("<a  class=\"shj_yindao_city\" >" + y.city[i].cities[j]['name'] + "</a>");
                    }
                }

            }
            if(value=="O"||value=="I"){
                $dom.css("marginLeft",'390px')
            }
        });
    };
    clicks();
